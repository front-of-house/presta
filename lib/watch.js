...
export async function watch (config, options = {}) {
  ...
  configWatcher.on('remove', async files => {
    if (files[0] === configFilepath) {
      /*
       * ensure configFilepath gets removed from dynamic entry
       *
       * @see https://github.com/sure-thing/presta/issues/43
       */
      updateDynamicEntry(dynamicIds, {
        ...config,
        configFilepath: null
      })
    }
  ...
  entryWatcher.on('all', async (event, file) => {
    ...
    if (event === 'add' && file === configFilepath) {
      ...
      /*
       * ensure configFilepath gets added to dynamic entry
       *
       * @see https://github.com/sure-thing/presta/issues/43
       */
      updateDynamicEntry(dynamicIds, {
