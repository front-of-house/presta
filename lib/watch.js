...
export async function watch (config, options = {}) {
   ...
   * create a file
   */
  const configPath = config.configFile || path.join(config.cwd, CONFIG_DEFAULT)
  ...
  const entryWatcher = chokidar.watch(
    config.pages.map(p => path.resolve(config.cwd, p)).map(globParent),
    { ignoreInitial: true }
  )
