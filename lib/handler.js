...
export function createHandler (router, userConfig) {
  return async (event, context) => {
    ...
    let onRequest = userConfig.onRequest
     ...
     */
    if (!page) {
      // if no page, but we have a global onRequest, try that first
      if (onRequest) {
        const response = onRequest(event, context)
     ...
     */
    onRequest = page.onRequest || onRequest
    if (onRequest) {
