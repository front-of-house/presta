...
export function filterUnique(arr) {
  ...
  outer: for (const a of arr) {
    for (const r of res) {
      if (a.name === r.name) continue outer;
    }
...
export function objectToTag(tag, props) {
  ...
  if (tag === "script") return `<script${attributes}>${props.children || ''}</script>`
  if (tag === "style") return `<style${attributes}>${props.children || ''}</style>`
...
export function createHeadTags(config = {}) {
  const { title, description, og, twitter, ...o } = merge(defaults, config);
